...
def get_url(sandbox=True):
    ...
    """
    use_sandbox = {
        True: "https://marketplace.ws.fd-recette.net/api.php/",
...
<auth xmlns='http://www.fnac.com/schemas/mp-dialog.xsd'>
  ...
  <key>{key}</key>
    ...
    """.format(
        partner_id=partner_id, shop_id=shop_id, key=key
    )
    url = get_url(sandbox)
...
def set_credentials(xml, sandbox=True):
    ...
    for cred, value in creds.items():
        xml = re.sub(
            pattern='{0}="[^"]+"'.format(cred),
            repl='{0}="{1}"'.format(cred, value),
            string=xml,
            flags=0,
        )
    return xml
